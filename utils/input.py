...
def get_args_train():
    ...
    parser.add_argument("--dropout", default=False, action="store_true", help="Activate dropout for the chosen model")
...
def get_model(args):
    ...
    if dropout is not None:
        if args.model == 'unet':
            model = UNet(dropout=dropout)
        elif args.model == 'u2net':
            model = U2Net(dropout=dropout)
        elif args.model == 'unet++':
            model = UNetPP(dropout=dropout)
        else:
